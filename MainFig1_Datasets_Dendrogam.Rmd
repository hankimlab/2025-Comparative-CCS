...
zf_mdk_trbc <- readRDS('/zf_mdk_trbc_20250515.rds')
...
fig <- DimPlot(zccs_nahia, cols=c('#861D21', '#F4C542'), group.by='sub_ccs', pt.size=2.6) & NoAxes() & NoLegend() & theme(plot.title=element_blank())
...
fig <- DimPlot(zccs_li, cols=c('#861D21', '#F4C542'), group.by='sub_ccs', pt.size=2) & NoAxes() & NoLegend() & theme(plot.title=element_blank())
...
ortholog_common_df_zf <- ortholog_df_zf %>%
  filter(
    ...
    mm %in% rownames(occs),
    mm %in% rownames(rccs),
    dr %in% intersect(rownames(zccs_nahia), intersect(rownames(zccs_li), rownames(zf_mdk_trbc)))
  )
...
process_ccs_dataset <- function(seurat_obj, ortholog_common_df, species_column, dataset_name) {
  ...
  } else if (dataset_name == "zccs_nahia") {
...
avg_zccs_nahia_expr <- process_ccs_dataset(zccs_nahia, ortholog_common_df_zf, species_column = 'dr', dataset_name = 'zccs_nahia')

zf_mdk_trbc$sub_ccs <- zf_mdk_trbc$cell_class
avg_zccs_li_expr <-
  process_ccs_dataset(
    JoinLayers(merge(zccs_li, zf_mdk_trbc)),
    ortholog_common_df_zf,
    species_column = 'dr',
    dataset_name = 'zccs_li'
  )
...
all_expr_data <- list(
  ...
  "kccs" = avg_kccs_expr,
  "rcss" = avg_rccs_expr,
  "zccs_nahia" = avg_zccs_nahia_expr,
  "zccs_li" = avg_zccs_li_expr
)
...
tiff('20250515_dendrogram_all_linear_pearson.tiff', 
